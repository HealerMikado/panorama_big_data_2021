...
- algorithmic analysis can confirm the profiling ( $O(n^2)$, $O(n\ln(n))$,  $O(p\ln(n))$, etc.)
...
- do you need these 100 columns? (in computation, in storage? do you need them simultaneouly?)
...
- comes at no cost with **associative symetrical operations** (like summation) but requires substantial rewriting for other algorithms
...
In computation, it is possible to perform **sparse matrix operations** very effectively.
...
**Command line** code, also known as **batch** or **shell**, is a way to take advantage of very effective low-level functions.
...
- with remote data, filter, select and sample at a distance
- if possible execute code distantly and return results
