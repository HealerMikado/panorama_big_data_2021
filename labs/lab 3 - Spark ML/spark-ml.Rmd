...
💡 **Q1.** You are given `errors`, a distributed vector of prediction errors. Write a map-and-reduce algorithm for computing the **total sum of squares**. _(You may want to create a Python version of this algorithm, using the `map(vector, function)` and `reduce(vector, function)` functions. `reduce` lives in the `functools` module. You may use lambda-functions.)_
...
listings_raw = spark.read.csv("s3://spark-lab-input-data-ensai20202021/airbnb/data-insideairbnb-australia-vic-barwon-south-west-vic-2020-05-26-data-listings.csv.gz", header=True, multiLine=True, escape='"')

listings_raw = listings_raw.withColumn("beds",     listings_raw["beds"    ].cast(IntegerType()))
listings_raw = listings_raw.withColumn("bedrooms", listings_raw["bedrooms"].cast(IntegerType()))
listings_raw = listings_raw.withColumn("time", listings_raw["last_scraped"].cast(DateType()))
listings_raw = listings_raw.withColumn(
    "price", substring(listings_raw["price"   ], pos=2, len=100).cast(FloatType())
)

listings = listings_raw.select(
    "id", "beds", "bedrooms", "price", "city"
)
listings = listings.dropna() # remove lines with missing values
```
...
**Q10.** Compute the average number of beds per property in Fairhaven in four different ways:
...
listings_beij = listings.filter(listings.neighbourhood=="Beijing")
listings_barc = listings.filter(listings.neighbourhood=="Barcelona")
